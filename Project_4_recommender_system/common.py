...
def prozent_error(X_gold, Y, mask_0):
    return np.sum(X_gold[mask_0] != Y[mask_0]) / np.sum(mask_0)
...
def bic(X: np.ndarray, mixture: GaussianMixture,
    ...
    n = X.shape[0]
    d = X.shape[1]
    p_number = K + K + K*d - 1 # var(K), p(K), mu(K*d), -1 because the parameters of p are dependent on the others
    return log_likelihood - ((1/2) * p_number * np.log(n))
