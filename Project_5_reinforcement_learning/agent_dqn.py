...
def epsilon_greedy(state_vector, epsilon):
    ...
    if np.random.random() <= epsilon:
        ...
        predicted_action = model(state_vector)
        action_index, object_index = torch.argmax(predicted_action[0]), torch.argmax(predicted_action[1])
        #print("Next_Qs: " + str(np.matmul(theta, state_vector)))
        #print("action to take: " + str(action_index) + ", " + str(object_index))
...
def deep_q_learning(current_state_vector, action_index, object_index, reward,
    ...
    #     q_values_actions_current, q_values_objects_current = model(current_state_vector)

    q_values_action_next, q_values_object_next = model(next_state_vector)
    q_values_actions_current, q_values_objects_current = model(current_state_vector)

    maxq_next = (1 / 2) * (q_values_action_next.max() + q_values_object_next.max())
    Expected_Q = reward + GAMMA * maxq_next * (1 - terminal)
    q_value_cur_state = (1 / 2) * (q_values_actions_current[action_index] +  q_values_objects_current[object_index])

    optimizer.zero_grad()
    loss = nn.MSELoss()
    output = loss(q_value_cur_state, Expected_Q/2)
    # It was loss.backward()
    output.backward()
    optimizer.step()
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        action_to_take_index, object_to_take_index = epsilon_greedy(current_state_vector, epsilon)
        ...
        if for_training:
            # update Q-function.
            deep_q_learning(current_state_vector, action_to_take_index, object_to_take_index, reward, next_state_vector, terminal)
            pass
        ...
        #     q_values_actions_current, q_values_objects_current = model(current_state_vector)
        #     q_value_cur_state = 1 / 2 * (torch.max(q_values_actions_current) + torch.max(q_values_objects_current))


        #     print("Terminal Q-Value: " + str(q_value_cur_state))
...
if __name__ == '__main__':
    ...
    model = DQN(state_dim, NUM_ACTIONS, NUM_OBJECTS, 100)
    ...
    plt.show()
    print("Average episodic reward Mean: " + str(np.mean(epoch_rewards_test[-1][-400:-1])))