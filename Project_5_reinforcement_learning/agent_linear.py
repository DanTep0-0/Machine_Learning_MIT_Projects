...
def epsilon_greedy(state_vector, theta, epsilon):
    ...
    if np.random.random() <= epsilon:
        ...
        action_object_index = np.argmax(np.matmul(theta, state_vector))
...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    Returns:
        None
    """
    Gradient_theta_negative = 0
    current_theta_index = tuple2index(action_index, object_index)
    
    if not terminal:

        Current_Q = np.dot(theta[current_theta_index], current_state_vector)
        Target_Q = reward + GAMMA * np.max(np.matmul(theta, next_state_vector))
        Gradient_theta_negative = (Target_Q - Current_Q) * current_state_vector


    theta[current_theta_index] = theta[current_theta_index] + ALPHA * Gradient_theta_negative
    #print("Theta Changes = " + str(np.sum(ALPHA * Gradient_theta_negative)))
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        action_to_take_index, object_to_take_index = epsilon_greedy(current_state_vector, theta, epsilon)
        ...
        if for_training:
            # update Q-function.
            linear_q_learning(theta, current_state_vector, action_to_take_index, object_to_take_index, reward, next_state_vector, terminal)
            pass
        ...
        #     print("Terminal Q-Value: " + str(np.max(np.matmul(theta, current_state_vector))))
...
if __name__ == '__main__':
    ...
    print("Average episodic reward Mean: " + str(np.mean(epoch_rewards_test[-1][-700:-1])))