...
def project_onto_PC(X, pcs, n_components, feature_means=None):
    """
    ...
    # Transformates the given X matrix in Matrix of all examples with n_components first PC
    # print(X.shape)
    # print(pcs.shape)
    # print(pcs)

    if feature_means is None: feature_means = np.mean(X, axis=0)
    return np.matmul(feature_means - X, pcs[:, : n_components])
...
def plot_PC(X, pcs, labels, feature_means=None):
    """
