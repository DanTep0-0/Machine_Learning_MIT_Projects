...
stopwords_connection = open('stopwords.txt')
stopword = stopwords_connection.read().split()
stopwords_connection.close()
...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    """
    # Your code here
    return max(-4324, 1 - (label * np.matmul(feature_vector, theta) + theta_0))

#print(hinge_loss_single(np.array([1, 2]), 1, np.array([1, 1]), 1))
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    loss_of_classifier = 0

    for i, row in enumerate(feature_matrix):
        loss_of_classifier += hinge_loss_single(row, labels[i], theta, theta_0)

    return loss_of_classifier / feature_matrix.shape[0]

#print(hinge_loss_full(np.array([[1, 2], [1, 2]]), [1, 1], np.array([1, 1]), 1))
...
def perceptron_single_step_update(
    ...
    #print("equasion = " + str(label * (np.matmul(feature_vector, current_theta) + current_theta_0)))
...
def average_perceptron(feature_matrix, labels, T):
    ...
    thetas = (np.zeros(feature_matrix.shape[1]), 0)
    thetas_0_sum = thetas[0]
    thetas_1_sum = thetas[1]
    ...
    return (thetas_0_sum / (T * feature_matrix.shape[0]), thetas_1_sum / (T * feature_matrix.shape[0]))
...
def pegasos(feature_matrix, labels, T, L):
    ...
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            iteration += 1
            thetas = pegasos_single_step_update(feature_matrix[i], labels[i], L, 1/(iteration ** 0.5), thetas[0], thetas[1])
...
def classify(feature_matrix, theta, theta_0):
    ...
    feature_classifications = np.zeros(feature_matrix.shape[0])

    for i, row in enumerate(feature_matrix):
        if np.matmul(row, theta) + theta_0 > 0:
            feature_classifications[i] = 1
        else :
            feature_classifications[i] = -1
...
def classifier_accuracy(
    ...
    thetas_training = classifier(train_feature_matrix, train_labels, **kwargs)

    training_classifications = classify(train_feature_matrix, thetas_training[0], thetas_training[1])
    val_classifications = classify(val_feature_matrix, thetas_training[0], thetas_training[1])

    train_accuracy = accuracy(training_classifications, train_labels)
    val_accuracy = accuracy(val_classifications, val_labels)

    #print(val_feature_matrix[val_classifications == -1][0:4])

    return train_accuracy, val_accuracy, val_classifications
...
def bag_of_words(texts, remove_stopword=True, occurence_threshold = 1):
    """
    ...
    for word, quantity in word_occurence.items():
        #print("word: " + word)
        #print("quantity: " + str(quantity))
        #print("threshold: " + str(occurence_threshold))
        if quantity >= occurence_threshold:
